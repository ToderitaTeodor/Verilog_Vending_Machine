...
class input_apb_transaction;
  ...
  function void post_randomize();
    ...
    if(pwrite) $display("Tranzactie de scriere: \t addr  = %0h\t wr_en = %0h\t wdata = %0h \t delay = %0d",addr,pwrite,data, delay_between_transaction);
    if(!pwrite) $display("Tranactie de citire: \t addr  = %0h\t wr_en = %0h\t wdata = %0h \t delay = %0d",addr,pwrite,data, delay_between_transaction);
    $display("-----------------------------------------");
  endfunction
  
  //operator de copiere a unui obiect intr-un alt obiect (deep copy)
  function input_apb_transaction do_copy();
